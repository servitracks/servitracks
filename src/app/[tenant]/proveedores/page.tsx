...
export default function ProveedoresPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Executive Dashboard — estilo SaaS ultra-compacto y 100% legible */}
      <div className={cn("grid gap-3", isWarehouse ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-5")}>
        {kpis.map((kpi) => (
          <div 
            key={kpi.label} 
            className="flex items-center gap-2.5 rounded-xl border border-neutral-200/80 p-3 bg-white shadow-2xs group cursor-pointer hover:border-neutral-300 hover:shadow-xs active:scale-[0.995] transition-all"
            onClick={() => setActiveTab(kpi.tab)}
          >
            <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg border flex-shrink-0 transition-transform group-hover:scale-105", kpi.bg, kpi.color)}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-baseline gap-1 mt-0.5 flex-wrap">
                <p className="text-sm font-black tracking-tight text-neutral-900 leading-tight">
                  {kpi.value}
                </p>
                {kpi.sub && <span className="text-[9px] font-semibold text-neutral-400">{kpi.sub}</span>}
              </div>
            </div>
          </div>
        ))}
