                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-xl border-neutral-100 p-2 shadow-lg w-52">
                      ...
                      {po.status !== "cancelada" && po.status !== "recibida_completa" && <DropdownMenuItem className="rounded-lg py-2 cursor-pointer gap-2 text-rose-600" onClick={() => changeStatus(po, "cancelada")}><XCircle className="h-4 w-4" /> Cancelar</DropdownMenuItem>}
                      {po.status === "borrador" && <DropdownMenuItem className="rounded-lg py-2 cursor-pointer gap-2 text-rose-600" onClick={() => { deletePurchaseOrder(po.id); toast.success("Orden eliminada"); }}><Trash2 className="h-4 w-4" /> Eliminar</DropdownMenuItem>}
                    </DropdownMenuContent>
                  </DropdownMenu>
                </TableCell>
              </TableRow>
