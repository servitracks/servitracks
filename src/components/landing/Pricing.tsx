...
function planToTier(plan: Plan) {
  return {
    ...
    description: `Plan ideal para tu taller con hasta ${plan.limite_empleados ?? "ilimitados"} técnicos y ${plan.limite_ordenes_mes ? plan.limite_ordenes_mes : "ilimitadas"} órdenes.`,
    features: [
      plan.limite_empleados ? `Hasta ${plan.limite_empleados} técnicos y usuarios` : "Técnicos y empleados ilimitados",
      plan.limite_ordenes_mes ? `Hasta ${plan.limite_ordenes_mes} órdenes de trabajo/mes` : "Órdenes de trabajo ilimitadas",
      plan.limite_sucursales ? `Hasta ${plan.limite_sucursales} sucursal(es)` : "Multi-sucursal ilimitada",
      plan.limite_whatsapp_mes ? `Hasta ${plan.limite_whatsapp_mes.toLocaleString()} msgs WhatsApp/mes` : "Mensajes WhatsApp ilimitados",
      plan.modulos.facturacion_fiscal ? "Facturación Electrónica e-CF / NCF (DGII)" : "Facturación POS estándar",
      plan.modulos.nomina_comisiones ? "Comisiones de Mecánicos & Nómina" : null,
      plan.modulos.inspecciones_mpi ? "Inspección Digital Multipunto (MPI)" : null,
      plan.modulos.proveedores_cuentas ? "Proveedores & Cuentas por Pagar" : null,
      plan.modulos.multisucursal ? "Panel Administrador Multi-Sede" : null,
      "Control de clientes, vehículos y caja chica",
      plan.destacado ? "Soporte prioritario por WhatsApp" : "Soporte estándar",
    ].filter(Boolean) as string[],
    cta: plan.destacado ? "Empezar con Plan " + plan.nombre : "Empezar prueba gratis",
